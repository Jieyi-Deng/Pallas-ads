...
Validation: both manifest formats; Codex model/tool invocation; Claude connected MCP with ten tools and three Skills; supported-Python and managed-Python setup; migration/rollback guards; three-media file reports across twelve process restarts; package integrity and runtime checks. Claude model-dialogue acceptance is deferred until the local Claude login is renewed. Fresh real-account and unattended scheduled-run acceptance remain separate.
